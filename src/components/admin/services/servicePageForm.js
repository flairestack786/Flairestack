...
export function seoToForm(row, serviceId) {
  ...
  return {
    ...
    robots: String(row.robots ?? 'index,follow').trim() || 'index,follow',
    og_title: row.og_title ?? '',
    og_description: row.og_description ?? '',
    og_type: String(row.og_type ?? 'website').trim() || 'website',
    twitter_card: String(row.twitter_card ?? 'summary_large_image').trim() || 'summary_large_image',
