...
function SectionHeader({ label, title, intro, id, light, centered = true }) {
  return (
    <motion.div
...
export default function ServicePageLayout({ service, page }) {
  ...
  return (
    <div className="sp-root" data-service={service.slug}>
      ...
      {/* ── Framework (light) ── */}
      {isOn('framework') && page.framework?.is_enabled !== false && (
      ...
      {/* ── Features bento (dark) ── */}
      {isOn('features') && page.features?.is_enabled !== false && (
      ...
      {isOn('growth') && page.growth?.is_enabled !== false && (
      ...
      {isOn('testimonials') && page.testimonials?.is_enabled !== false && (
        <Band
          tone="light"
          id="testimonials"
          className={testimonialsBackground ? 'sp-band--has-bg' : ''}
        >
          {testimonialsBackground && (
            <SectionBackground image={testimonialsBackground} className="sp-band-bg" />
          )}
          <motion.div
          ...
          >
            ...
            />
            <ServiceTestimonialsSlider testimonials={testimonialItems} />
          </motion.div>
        </Band>
      )}

      {/* ── Process (light) ── */}
      {isOn('process') && page.process?.is_enabled !== false && (
            ...
            <div className="sp-process-timeline sp-process-timeline--premium">
              {(page.process.steps ?? []).map((step, i) => (
                ...
                >
                  ...
                  {step.image?.src && (
                    <div className="sp-process-step-media">
                      <img
                        src={step.image.src}
                        alt={step.image.alt || step.title || ''}
                        loading="lazy"
                        decoding="async"
                        className="sp-process-step-img"
                      />
                    </div>
                  )}
                  <h3>{step.title}</h3>
      ...
      {/* ── Industries (dark) ── */}
      {isOn('industries') && page.industries?.is_enabled !== false && (
      ...
      {/* ── Post-launch (dark) ── */}
      {isOn('post_launch') && page.postLaunch?.is_enabled !== false && (
        <Band tone="dark">
          <motion.div
            initial="hidden"
            whileInView="visible"
      ...
      {/* ── FAQ (light) ── */}
      {isOn('faq') && page.faq?.is_enabled !== false && (
        <Band tone="light" id="faq">
          <motion.div
            ...
            variants={staggerContainer}
          >
            <header className="sp-section-header sp-section-header--light sp-section-header--center sp-faq-header">
      ...
      {/* ── Final CTA ── */}
      {isOn('final_cta') && page.finalCta?.is_enabled !== false && (
        <motion.section
